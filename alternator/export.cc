...
//   - formatter (`export_pipeline_interface`) - serializes rjson::value to simple binary format (JSON lines, Ion, CSV are required by Amazon specs),
...
//   - parser - deserializes binary data to rjson::value (JSON lines, Ion, CSV are required by Amazon specs),
//   - decompressor - optionally decompresses the data - Amazon S3 requires support for gzip compression,
//   - reader (`import_pipeline_interface`) - reads the data from the storage (e.g. S3 or in-memory).
// After construction user is expected to call `import_pipeline_interface::read()`. This will read some data from the source
// and call `decompression_interface::decompress()` with it, which will - optionally - decompress it,
// then call `parsing_interface::parse()` with the decompressed data. The parser invokes the on_item callback
...
// In memory sink - stores data to a caller owned in_memory_debug_storage buffer object.
// Single threaded, single "file" use only. Caller must ensure in_memory_debug_storage object lives long enough.
class in_memory_storage_sink : public storage_sink_interface {
    in_memory_debug_storage& _storage;
public:
    explicit in_memory_storage_sink(in_memory_debug_storage& storage)
        : _storage(storage) {}
...
// In memory source object - reads data from a caller owned in_memory_debug_storage buffer object and
...
// Single threaded, single "file" use only. Caller must ensure in_memory_debug_storage object lives long enough, and that on_item callback remains valid until flush_and_close() is called.
class in_memory_source : public import_pipeline_interface {
    in_memory_debug_storage& _storage;
    std::unique_ptr<decompression_interface> _decompressor;
public:
    in_memory_source(in_memory_debug_storage& storage,
    ...
    seastar::future<> read() override {
        ...
        for (size_t i = 0; i < data.size(); i += chunk_size) {
            auto n = std::min(chunk_size, data.size() - i);
            co_await _decompressor->decompress(std::as_bytes(std::span<const char>(ptr + i, n)));
        }
    }
...
std::unique_ptr<export_pipeline_interface> create_in_memory_sink_pipeline(in_memory_debug_storage& storage) {
...
std::unique_ptr<import_pipeline_interface> create_in_memory_source_pipeline(in_memory_debug_storage& storage, std::function<seastar::future<>(rjson::value)> on_item) {
