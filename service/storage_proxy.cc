...
{
    if (utils::get_local_injector().enter_if_parameter_equals("alternator_query_result_timeout", "table_name", query_schema->cf_name())) {
        return make_ready_future<result<coordinator_query_result>>(
            // Note: we don't care about the actual values of the exception's fields, since this is just for testing Alternator's error handling.
